...
const sbg_driver::msg::SbgEkfStatus MessageWrapper::createEkfStatusMessage(uint32_t ekf_status)
const
{
...
const sbg_driver::msg::SbgGpsPosStatus MessageWrapper::createGpsPosStatusMessage(
  const SbgLogGpsPos & ref_log_gps_pos) const
{
...
const sbg_driver::msg::SbgGpsVelStatus MessageWrapper::createGpsVelStatusMessage(
  const SbgLogGpsVel & ref_log_gps_vel) const
{
...
const sbg_driver::msg::SbgImuStatus MessageWrapper::createImuStatusMessage(uint16_t sbg_imu_status)
const
{
...
const sbg_driver::msg::SbgMagStatus MessageWrapper::createMagStatusMessage(
  const SbgLogMag & ref_log_mag) const
{
...
const sbg_driver::msg::SbgShipMotionStatus MessageWrapper::createShipMotionStatusMessage(
  const SbgLogShipMotionData & ref_log_ship_motion) const
{
  ...
  ship_motion_status_message.heave_vel_aided =
    (ref_log_ship_motion.status & SBG_ECOM_HEAVE_VEL_AIDED) != 0;
  ship_motion_status_message.period_available =
    (ref_log_ship_motion.status & SBG_ECOM_HEAVE_PERIOD_INCLUDED) != 0;
  ship_motion_status_message.period_valid =
    (ref_log_ship_motion.status & SBG_ECOM_HEAVE_PERIOD_VALID) != 0;

  return ship_motion_status_message;
}

const sbg_driver::msg::SbgStatusAiding MessageWrapper::createStatusAidingMessage(
  const SbgLogStatusData & ref_log_status) const
{
  sbg_driver::msg::SbgStatusAiding status_aiding_message;

  status_aiding_message.gps1_pos_recv =
    (ref_log_status.aidingStatus & SBG_ECOM_AIDING_GPS1_POS_RECV) != 0;
  status_aiding_message.gps1_vel_recv =
    (ref_log_status.aidingStatus & SBG_ECOM_AIDING_GPS1_VEL_RECV) != 0;
  status_aiding_message.gps1_hdt_recv =
    (ref_log_status.aidingStatus & SBG_ECOM_AIDING_GPS1_HDT_RECV) != 0;
  status_aiding_message.gps1_utc_recv =
    (ref_log_status.aidingStatus & SBG_ECOM_AIDING_GPS1_UTC_RECV) != 0;
...
const sbg_driver::msg::SbgStatusCom MessageWrapper::createStatusComMessage(
  const SbgLogStatusData & ref_log_status) const
{
...
const sbg_driver::msg::SbgStatusGeneral MessageWrapper::createStatusGeneralMessage(
  const SbgLogStatusData & ref_log_status) const
{
  sbg_driver::msg::SbgStatusGeneral status_general_message;

  status_general_message.main_power =
    (ref_log_status.generalStatus & SBG_ECOM_GENERAL_MAIN_POWER_OK) != 0;
  status_general_message.imu_power =
    (ref_log_status.generalStatus & SBG_ECOM_GENERAL_IMU_POWER_OK) != 0;
  status_general_message.gps_power =
    (ref_log_status.generalStatus & SBG_ECOM_GENERAL_GPS_POWER_OK) != 0;
  status_general_message.settings = (ref_log_status.generalStatus & SBG_ECOM_GENERAL_SETTINGS_OK) !=
    0;
  status_general_message.temperature =
    (ref_log_status.generalStatus & SBG_ECOM_GENERAL_TEMPERATURE_OK) != 0;

  return status_general_message;
}

const sbg_driver::msg::SbgUtcTimeStatus MessageWrapper::createUtcStatusMessage(
  const SbgLogUtcData & ref_log_utc) const
{
  ...
  utc_status_message.clock_status =
    static_cast<uint8_t>(sbgEComLogUtcGetClockStatus(ref_log_utc.status));
  utc_status_message.clock_utc_status =
    static_cast<uint8_t>(sbgEComLogUtcGetClockUtcStatus(ref_log_utc.status));
...
const rclcpp::Time MessageWrapper::convertUtcTimeToUnix(
  const sbg_driver::msg::SbgUtcTime & ref_sbg_utc_msg) const
{
  rclcpp::Time utc_to_epoch;
  uint32_t days;
  uint64_t nanoseconds;
...
const sbg_driver::msg::SbgAirDataStatus MessageWrapper::createAirDataStatusMessage(
  const SbgLogAirData & ref_sbg_air_data) const
{
  sbg_driver::msg::SbgAirDataStatus air_data_status_message;

  air_data_status_message.is_delay_time =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_TIME_IS_DELAY) != 0;
  air_data_status_message.pressure_valid =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_PRESSURE_ABS_VALID) != 0;
  air_data_status_message.altitude_valid =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_ALTITUDE_VALID) != 0;
  air_data_status_message.pressure_diff_valid =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_PRESSURE_DIFF_VALID) != 0;
  air_data_status_message.air_speed_valid =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_AIRPSEED_VALID) != 0;
  air_data_status_message.air_temperature_valid =
    (ref_sbg_air_data.status & SBG_ECOM_AIR_DATA_TEMPERATURE_VALID) != 0;
...
void MessageWrapper::initUTM(double Lat, double Long, double altitude)
{
  ...
  RCLCPP_INFO(
    rclcpp::get_logger(
      "Message wrapper"), "initialized from lat:%f long:%f UTM zone %d%c: easting:%fm (%dkm) northing:%fm (%dkm)", Lat, Long, m_utm0_.zone, UTMLetterDesignator(
      Lat), m_utm0_.easting, (int)(m_utm0_.easting) / 1000, m_utm0_.northing,
 ...
 */
void MessageWrapper::LLtoUTM(
  double Lat, double Long, int zoneNumber, double & UTMNorthing,
  double & UTMEasting) const
{
...
void MessageWrapper::setUseManualDatum(bool manual_datum)
{
...
const sbg_driver::msg::SbgEkfEuler MessageWrapper::createSbgEkfEulerMessage(
  const SbgLogEkfEulerData & ref_log_ekf_euler) const
{
...
const sbg_driver::msg::SbgEkfNav MessageWrapper::createSbgEkfNavMessage(
  const SbgLogEkfNavData & ref_log_ekf_nav) const
{
...
const sbg_driver::msg::SbgEkfQuat MessageWrapper::createSbgEkfQuatMessage(
  const SbgLogEkfQuatData & ref_log_ekf_quat) const
{
  ...
  if (m_use_enu_) {
    ...
    tf2::Quaternion q(ref_log_ekf_quat.quaternion[1], ref_log_ekf_quat.quaternion[2],
      ref_log_ekf_quat.quaternion[3], ref_log_ekf_quat.quaternion[0]);

    tf2::Quaternion convert(factor, factor, 0, 0);
    q *= convert;

    ekf_quat_message.quaternion.w = q.getW();
    ekf_quat_message.quaternion.x = q.getX();
    ekf_quat_message.quaternion.y = q.getY();
    ekf_quat_message.quaternion.z = q.getZ();
  } else {
...
const sbg_driver::msg::SbgEvent MessageWrapper::createSbgEventMessage(
  const SbgLogEvent & ref_log_event) const
{
...
const sbg_driver::msg::SbgGpsHdt MessageWrapper::createSbgGpsHdtMessage(
  const SbgLogGpsHdt & ref_log_gps_hdt) const
{
...
const sbg_driver::msg::SbgGpsPos MessageWrapper::createSbgGpsPosMessage(
  const SbgLogGpsPos & ref_log_gps_pos) const
{
...
const sbg_driver::msg::SbgGpsRaw MessageWrapper::createSbgGpsRawMessage(
  const SbgLogGpsRaw & ref_log_gps_raw) const
{
  sbg_driver::msg::SbgGpsRaw gps_raw_message;

  gps_raw_message.data.assign(
    ref_log_gps_raw.rawBuffer,
    ref_log_gps_raw.rawBuffer + ref_log_gps_raw.bufferSize);

  return gps_raw_message;
}

const sbg_driver::msg::SbgGpsVel MessageWrapper::createSbgGpsVelMessage(
  const SbgLogGpsVel & ref_log_gps_vel) const
{
...
const sbg_driver::msg::SbgImuData MessageWrapper::createSbgImuDataMessage(
  const SbgLogImuData & ref_log_imu_data) const
{
...
const sbg_driver::msg::SbgMag MessageWrapper::createSbgMagMessage(const SbgLogMag & ref_log_mag)
const
{
...
const sbg_driver::msg::SbgMagCalib MessageWrapper::createSbgMagCalibMessage(
  const SbgLogMagCalib & ref_log_mag_calib) const
{
...
const sbg_driver::msg::SbgOdoVel MessageWrapper::createSbgOdoVelMessage(
  const SbgLogOdometerData & ref_log_odo) const
{
...
const sbg_driver::msg::SbgShipMotion MessageWrapper::createSbgShipMotionMessage(
  const SbgLogShipMotionData & ref_log_ship_motion) const
{
...
const sbg_driver::msg::SbgStatus MessageWrapper::createSbgStatusMessage(
  const SbgLogStatusData & ref_log_status) const
{
...
const sbg_driver::msg::SbgUtcTime MessageWrapper::createSbgUtcTimeMessage(
  const SbgLogUtcData & ref_log_utc)
{
  ...
  if (!m_first_valid_utc_) {
    if (utc_time_message.clock_status.clock_stable &&
      utc_time_message.clock_status.clock_utc_sync)
    {
...
const sbg_driver::msg::SbgAirData MessageWrapper::createSbgAirDataMessage(
  const SbgLogAirData & ref_air_data_log) const
{
...
const sbg_driver::msg::SbgImuShort MessageWrapper::createSbgImuShortMessage(
  const SbgLogImuShort & ref_short_imu_log) const
{
  ...
  const sbg_driver::msg::SbgEkfQuat & ref_sbg_quat_msg) const
{
  ...
  imu_ros_message.orientation_covariance[0] = ref_sbg_quat_msg.accuracy.x *
    ref_sbg_quat_msg.accuracy.x;
  imu_ros_message.orientation_covariance[4] = ref_sbg_quat_msg.accuracy.y *
    ref_sbg_quat_msg.accuracy.y;
  imu_ros_message.orientation_covariance[8] = ref_sbg_quat_msg.accuracy.z *
    ref_sbg_quat_msg.accuracy.z;
  ...
  const sbg_driver::msg::SbgEkfEuler & ref_sbg_euler_msg) const
{
  ...
  q.setRPY(
    ref_sbg_euler_msg.angle.x,
    ref_sbg_euler_msg.angle.y,
    ref_sbg_euler_msg.angle.z);

  imu_ros_message.orientation = tf2::toMsg(q);
  imu_ros_message.angular_velocity = ref_sbg_imu_msg.delta_angle;
  imu_ros_message.linear_acceleration = ref_sbg_imu_msg.delta_vel;


  imu_ros_message.orientation_covariance[0] = ref_sbg_euler_msg.accuracy.x *
    ref_sbg_euler_msg.accuracy.x;
  imu_ros_message.orientation_covariance[4] = ref_sbg_euler_msg.accuracy.y *
    ref_sbg_euler_msg.accuracy.y;
  imu_ros_message.orientation_covariance[8] = ref_sbg_euler_msg.accuracy.z *
    ref_sbg_euler_msg.accuracy.z;
...
const nav_msgs::msg::Odometry MessageWrapper::createRosOdoMessage(
  ...
  const sbg_driver::msg::SbgEkfQuat & ref_ekf_quat_msg,
  const sbg_driver::msg::SbgEkfEuler & ref_ekf_euler_msg)
{
  tf2::Quaternion orientation(ref_ekf_quat_msg.quaternion.x, ref_ekf_quat_msg.quaternion.y,
    ref_ekf_quat_msg.quaternion.z, ref_ekf_quat_msg.quaternion.w);
  ...
  const sbg_driver::msg::SbgEkfEuler & ref_ekf_euler_msg)
{
  ...
  if (t_use_2d_) {
    orientation.setRPY(0, 0, ref_ekf_euler_msg.angle.z);
  } else {
    orientation.setRPY(
      ref_ekf_euler_msg.angle.x, ref_ekf_euler_msg.angle.y,
  ...
  const sbg_driver::msg::SbgEkfEuler & ref_ekf_euler_msg)
{
  ...
  LLtoUTM(
    ref_ekf_nav_msg.latitude, ref_ekf_nav_msg.longitude, m_utm0_.zone, utm_easting,
    utm_northing);
  ...
  if (t_use_2d_) {
    odo_ros_msg.pose.pose.position.z = 0;
  } else {
  ...
  odo_ros_msg.pose.covariance[3 * 6 + 3] = ref_ekf_euler_msg.accuracy.x *
    ref_ekf_euler_msg.accuracy.x;
  odo_ros_msg.pose.covariance[4 * 6 + 4] = ref_ekf_euler_msg.accuracy.y *
    ref_ekf_euler_msg.accuracy.y;
  odo_ros_msg.pose.covariance[5 * 6 + 5] = ref_ekf_euler_msg.accuracy.z *
    ref_ekf_euler_msg.accuracy.z;
  ...
  odo_ros_msg.twist.covariance[0 * 6 + 0] = ref_ekf_nav_msg.velocity_accuracy.x *
    ref_ekf_nav_msg.velocity_accuracy.x;
  odo_ros_msg.twist.covariance[1 * 6 + 1] = ref_ekf_nav_msg.velocity_accuracy.y *
    ref_ekf_nav_msg.velocity_accuracy.y;
  odo_ros_msg.twist.covariance[2 * 6 + 2] = ref_ekf_nav_msg.velocity_accuracy.z *
    ref_ekf_nav_msg.velocity_accuracy.z;
  ...
  if (m_odom_publish_tf_) {
    ...
    fillTransform(
      odo_ros_msg.header.frame_id, m_odom_base_frame_id_, odo_ros_msg.pose.pose,
      transform);
...
const sensor_msgs::msg::Temperature MessageWrapper::createRosTemperatureMessage(
  const sbg_driver::msg::SbgImuData & ref_sbg_imu_msg) const
{
...
const sensor_msgs::msg::MagneticField MessageWrapper::createRosMagneticMessage(
  const sbg_driver::msg::SbgMag & ref_sbg_mag_msg) const
{
...
const geometry_msgs::msg::TwistStamped MessageWrapper::createRosTwistStampedMessage(
  const sbg_driver::msg::SbgEkfEuler & ref_sbg_ekf_euler_msg,
  const sbg_driver::msg::SbgEkfNav & ref_sbg_ekf_nav_msg,
  const sbg_driver::msg::SbgImuData & ref_sbg_imu_msg) const
{
  sbg::SbgMatrix3f tdcm;
  tdcm.makeDcm(
    sbg::SbgVector3f(
      ref_sbg_ekf_euler_msg.angle.x, ref_sbg_ekf_euler_msg.angle.y,
      ref_sbg_ekf_euler_msg.angle.z));
  tdcm.transpose();

  const sbg::SbgVector3f res = tdcm * sbg::SbgVector3f(
    ref_sbg_ekf_nav_msg.velocity.x,
    ref_sbg_ekf_nav_msg.velocity.y,
    ref_sbg_ekf_nav_msg.velocity.z);

  return createRosTwistStampedMessage(res, ref_sbg_imu_msg);
}

const geometry_msgs::msg::TwistStamped MessageWrapper::createRosTwistStampedMessage(
  const sbg_driver::msg::SbgEkfQuat & ref_sbg_ekf_quat_msg,
  const sbg_driver::msg::SbgEkfNav & ref_sbg_ekf_nav_msg,
  const sbg_driver::msg::SbgImuData & ref_sbg_imu_msg) const
{
  sbg::SbgMatrix3f tdcm;
  tdcm.makeDcm(
    ref_sbg_ekf_quat_msg.quaternion.w, ref_sbg_ekf_quat_msg.quaternion.x,
    ref_sbg_ekf_quat_msg.quaternion.y, ref_sbg_ekf_quat_msg.quaternion.z);
  tdcm.transpose();
...
const geometry_msgs::msg::TwistStamped MessageWrapper::createRosTwistStampedMessage(
  const sbg::SbgVector3f & body_vel, const sbg_driver::msg::SbgImuData & ref_sbg_imu_msg) const
{
...
const geometry_msgs::msg::PointStamped MessageWrapper::createRosPointStampedMessage(
  const sbg_driver::msg::SbgEkfNav & ref_sbg_ekf_msg) const
{
  ...
  prime_vertical_radius = equatorial_radius / sqrt(
    1.0 - pow(eccentricity, 2) * pow(
      sin(
        latitude), 2));

  point_stamped_message.point.x = (prime_vertical_radius + ref_sbg_ekf_msg.altitude) *
    cos(latitude) * cos(longitude);
  point_stamped_message.point.y = (prime_vertical_radius + ref_sbg_ekf_msg.altitude) *
    cos(latitude) * sin(longitude);
  point_stamped_message.point.z =
    ((pow(
      polar_radius,
      2) / pow(equatorial_radius, 2)) * prime_vertical_radius + ref_sbg_ekf_msg.altitude) * sin(
    latitude);

  return point_stamped_message;
}

const sensor_msgs::msg::TimeReference MessageWrapper::createRosUtcTimeReferenceMessage(
  const sbg_driver::msg::SbgUtcTime & ref_sbg_utc_msg) const
{
...
const sensor_msgs::msg::NavSatFix MessageWrapper::createRosNavSatFixMessage(
  const sbg_driver::msg::SbgGpsPos & ref_sbg_gps_msg) const
{
  ...
  nav_sat_fix_message.position_covariance[0] = ref_sbg_gps_msg.position_accuracy.x *
    ref_sbg_gps_msg.position_accuracy.x;
  nav_sat_fix_message.position_covariance[4] = ref_sbg_gps_msg.position_accuracy.y *
    ref_sbg_gps_msg.position_accuracy.y;
  nav_sat_fix_message.position_covariance[8] = ref_sbg_gps_msg.position_accuracy.z *
    ref_sbg_gps_msg.position_accuracy.z;
...
const sensor_msgs::msg::FluidPressure MessageWrapper::createRosFluidPressureMessage(
  const sbg_driver::msg::SbgAirData & ref_sbg_air_msg) const
{
...
bool MessageWrapper::getUse2D(void) const
{
  return t_use_2d_;
}
